...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.Image = field.NewString(tableName, "image")
	_user.Phone = field.NewString(tableName, "phone")
	_user.Email = field.NewString(tableName, "email")
	_user.Password = field.NewString(tableName, "password")
	_user.PositionID = field.NewUint(tableName, "position_id")
	_user.RoleID = field.NewUint(tableName, "role_id")
	_user.OrganizationID = field.NewUint(tableName, "organization_id")
	_user.Deleted = field.NewBool(tableName, "deleted")
	_user.Fcm = field.NewString(tableName, "fcm")
	_user.Description = field.NewString(tableName, "description")
	_user.Position = userBelongsToPosition{
...
type user struct {
	...
	Position       userBelongsToPosition
...
func (u *user) updateTableName(table string) *user {
	...
	u.PositionID = field.NewUint(table, "position_id")
	u.RoleID = field.NewUint(table, "role_id")
	u.OrganizationID = field.NewUint(table, "organization_id")
	u.Deleted = field.NewBool(table, "deleted")
	u.Fcm = field.NewString(table, "fcm")
...
type userBelongsToPosition struct {
	db *gorm.DB

	field.RelationField
}

func (a userBelongsToPosition) Where(conds ...field.Expr) *userBelongsToPosition {
	if len(conds) == 0 {
		return &a
	}
...
func (a userBelongsToPosition) WithContext(ctx context.Context) *userBelongsToPosition {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a userBelongsToPosition) Session(session *gorm.Session) *userBelongsToPosition {
	a.db = a.db.Session(session)
	return &a
}

func (a userBelongsToPosition) Model(m *models.User) *userBelongsToPositionTx {
	return &userBelongsToPositionTx{a.db.Model(m).Association(a.Name())}
}

type userBelongsToPositionTx struct{ tx *gorm.Association }

func (a userBelongsToPositionTx) Find() (result *models.Position, err error) {
	return result, a.tx.Find(&result)
}

func (a userBelongsToPositionTx) Append(values ...*models.Position) (err error) {
...
func (a userBelongsToPositionTx) Replace(values ...*models.Position) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userBelongsToPositionTx) Delete(values ...*models.Position) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userBelongsToPositionTx) Clear() error {
	return a.tx.Clear()
}

func (a userBelongsToPositionTx) Count() int64 {
	return a.tx.Count()
}
